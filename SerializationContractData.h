// Serialization and unserialization of STL data structures.
...
namespace SerializationContract {
    struct Serializer {
        Serializer(std::vector<uint8_t>& bytes) : bytes_(bytes) { bytes_.clear(); }
        ...
        template <typename T>
        Serializer& String(const T& arg) {
            Serializer& serializer = *this;

            for (auto c : arg) {
                serializer.Serialize(c);
            }
            serializer.Serialize(std::remove_reference_t<decltype(arg[0])>{});
    ...
    struct Unserializer {
        Unserializer(const std::vector<uint8_t>& bytes) : bytes_(bytes) {}
        ...
        Unserializer& SequenceContainer(T& arg) {
            ...
            using type = std::remove_reference_t<decltype(arg[0])>;
